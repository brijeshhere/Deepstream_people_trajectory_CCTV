...
# This program draw the trajectory path of the people walking 
# Use CAse: the location of adviretisement hording can be known
# Use case : Area in the store section with high demand
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            try:
                ...
                if frame_number%5==0:
                    if obj_meta.object_id in tracker_trajectory.keys():
                        tracker_trajectory[obj_meta.object_id].append((centroid_x,centroid_y))
                    else:
                        tracker_trajectory[obj_meta.object_id]=[]
        ...
        params_count=0
        for key in tracker_trajectory.keys():
            walk_trajectory_key=[]
            for num_coord in range(len(tracker_trajectory[key])-1):
                if params_count%15==0:
                    params_count=0
                    display_meta=pyds.nvds_acquire_display_meta_from_pool(batch_meta)
                    display_meta.num_labels += 1
                params_count+=1
                walk_trajectory_key.append(display_meta.line_params[params_count])
                walk_trajectory_key[num_coord].x1=tracker_trajectory[key][num_coord][0]
                walk_trajectory_key[num_coord].y1=tracker_trajectory[key][num_coord][1]
                walk_trajectory_key[num_coord].x2=tracker_trajectory[key][num_coord+1][0]
                walk_trajectory_key[num_coord].y2=tracker_trajectory[key][num_coord+1][1]
                walk_trajectory_key[num_coord].line_width=2
                walk_trajectory_key[num_coord].line_color.red=1.0
                walk_trajectory_key[num_coord].line_color.green=0.0
                walk_trajectory_key[num_coord].line_color.blue=0.0
                walk_trajectory_key[num_coord].line_color.alpha=1.0
                # print(f"{key=} {num_coord=} {tracker_trajectory[key][num_coord]=}")
